...
create_tunnel(
        ...
        sai_ip4_t ip,
        sai_object_id_t underlay_rif = 0x0)
{
    sai_attribute_t attr;
...
bool VxlanTunnel::createTunnel(map_type encap, map_type decap)
{
    try
    {
        ...
        ids.tunnel_id        = create_tunnel(ids.tunnel_encap_id, ids.tunnel_decap_id, 0x0);
        ids.tunnel_term_id   = create_tunnel_termination(ids.tunnel_id, src_ip.getV4Addr(),
                                                       dst_ip.getV4Addr(), gVirtualRouterId);
        active = true;
    }
...
bool VnetIntfOrch::addOperation(const Request& request)
{
    ...
    const auto& if_name = request.getKeyString(0);
    SWSS_LOG_INFO("If name '%s'", if_name.c_str());

    const auto& ip_addr = request.getKeyIpAddress(1);
    SWSS_LOG_INFO("Ip prefix '%s'", ip_addr.to_string().c_str());


    SWSS_LOG_NOTICE("Vxlan interface %s created for %s", if_name.c_str(), vnet_name.c_str());

    return true;
}

bool VnetIntfOrch::delOperation(const Request& request)
{
    SWSS_LOG_ENTER();
